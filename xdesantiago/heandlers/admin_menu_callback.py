...
async def list_data_films(call: types.Message):
    ...
    for i in await get_AllChennel():
        file_films.write(f'Идентификатор(вводить при удалении): {i[0]}, Отображение: {i[1]}, Сыллка: {i[2]}\n')
    file_films.close()
    try:
...
async def add_film_admin(call: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=call.from_user.id, text='Хорошо отправь мне код🔑', reply_markup=types.InlineKeyboardMarkup(row_width=1).\
        add(types.InlineKeyboardButton(text='Сгенерировать♻️', callback_data='generetion_fims_code_admin'), ikb_back_oikb))
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
...
async def delete_film_admin(message: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=message.from_user.id, text='Хорошо отправь мне код фильма которого хочешь удалить🗑', reply_markup=ikb_back)
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
...
async def add_chennel_admin(message: types.Message, state: FSMContext):
    message_data=await bot.send_message(chat_id=message.from_user.id, text='Хорошо дайте в канале права мне "просматривать участников" и "пригласительные ссылками", после отправь мне @username или id канала которого хотите добавить➕', reply_markup=ikb_back)
    async with state.proxy() as data:
        data['message_id']=message_data.message_id
            ...
            for e in me_chat_status:
                if e['user']['username'] == me_username:
                    if e['can_invite_users']:
                        ...
                        text+=f'Канал: {i[1]} не имею доступ к ссылкам❗️\n\n'
