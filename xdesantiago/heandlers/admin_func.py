...
async def myling_list(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        count_accept=0
        count_error=0
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=f'*Данные о рассылке\nТекст: "{message.text}"\n✅Успешно: {count_accept}\n❌Ошибки: {count_error}*', parse_mode=types.ParseMode.MARKDOWN)
        for i in data_user:
            try:
                await bot.copy_message(chat_id=i, from_chat_id=message.from_user.id, message_id=msg_myling, parse_mode=types.ParseMode.MARKDOWN)
                count_accept+=1
                await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=f'*Данные о рассылке\nТекст: "{message.text}"\n✅Успешно: {count_accept}\n❌Ошибки: {count_error}*', parse_mode=types.ParseMode.MARKDOWN)
            except:
                count_error+=1
        ...
        await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=f'*Данные о рассылке\nТекст: "{message.text}"\n✅Успешно: {count_accept}\n❌Ошибки: {count_error}\nРассылка завершена🔔*', parse_mode=types.ParseMode.MARKDOWN, reply_markup=ikb_close)
    await state.finish()
...
async def delete_film(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if await delete_Film(code=chennel_identifier):
            ...
            try:
                await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Нет такого кода❌', reply_markup=ikb_back)
            except:
                pass
...
async def delete_chennel(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        if await delete_Chennel(chennel_identifier=message.text):
            ...
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Извините вы не добавляли такого канала ❌', reply_markup=ikb_back)
...
async def chennger_wellcome_text(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        try:
            ...
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Успешно изменил текст приветствия✅', reply_markup=ikb_close)
            await state.finish()
            await update_wellcome_text(text_type='wellcome', text=message.text)
        except:
            try:
                await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Неправильная разметка MARKDOWN✂️', reply_markup=ikb_back)
            except:
                pass
...
async def chennger_wellcome_text(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        try:
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text=message.text, parse_mode=types.ParseMode.MARKDOWN)
            await bot.edit_message_text(chat_id=message.from_user.id, message_id=data['message_id'], text='Успешно изменил текст фильма✅', reply_markup=ikb_close)
