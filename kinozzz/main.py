...
async def send_welcome(message: types.Message, state: FSMContext):
    ...
    text = file.read()
    if not str(message.from_user.id) in text and check_sub_channel(await bot.get_chat_member(chat_id=chatid, user_id=message.from_user.id)):
        ...
        await bot.send_message(message.from_user.id, Notsub_message, reply_markup=forsubkeyboard)

@dp.callback_query_handler(text="subchanneldone")
async def subchanneldone(message: types.Message):
    await bot.delete_message(message.from_user.id, message.message.message_id)
    if check_sub_channel(await bot.get_chat_member(chat_id=chatid, user_id=message.from_user.id)):
        text = f'<a href="https://bot.kinozzz.ru/poster/main.png">🎞️</a> <b><u>Kinozzz Bot</u></b> — уникальный в своём роде бот, который даст Вам возможность <b>бесплатно</b> наслаждаться новинками <b>Российского</b> и <b>зарубежного</b> кинематографа с <u>любого устройства</u>.\n\nБОЛЬШЕ БОТОВ НА CONFF.ORG'
        await bot.send_message(message.from_user.id, f'{text}', reply_markup=inlinekeyboard)
    else:
        await bot.send_message(message.from_user.id, Notsub_message, reply_markup=forsubkeyboard)
